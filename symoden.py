...
class SymODEN_R(torch.nn.Module):
    ...
    def time_derivative(self, x):
        with torch.enable_grad():
            ...
            self.nfe += 1
            bs = x.shape[0]
            zero_vec = torch.zeros(bs, int(x.shape[-1]/3), dtype=torch.float32, device =self.device)
            ...
            H_vector_field = torch.cat((H_vector_field, zero_vec), dim=1)
...
class PixelSymODEN_R(torch.nn.Module):
    ...
    def time_derivative(self, z):
        return self.symoden.time_derivative(z)
