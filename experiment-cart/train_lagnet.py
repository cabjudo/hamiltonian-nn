...
def pixelhnn_loss(x, x_next, ctrl, model, loss, device, return_scalar=True):
...
def train(args):
  ...
  for step in tqdm(range(args.total_steps+1)):
    ...
    loss = pixelhnn_loss(x[ixs], next_x[ixs], ctrl[ixs], model, criterion, device)
    ...
    if args.verbose and step % args.print_every == 0:
      ...
      test_loss = pixelhnn_loss(test_x[test_ixs], test_next_x[test_ixs], test_ctrl[test_ixs],  model, criterion, device)
  ...
  train_dist1, train_dist2 = tee( pixelhnn_loss(x[i].unsqueeze(0), next_x[i].unsqueeze(0), ctrl[i].unsqueeze(0), model, criterion, device).detach().cpu().numpy() for i in train_ind )
  ...
  test_dist1, test_dist2 = tee( pixelhnn_loss(test_x[i].unsqueeze(0), test_next_x[i].unsqueeze(0), test_ctrl[i].unsqueeze(0), model, criterion, device).detach().cpu().numpy() for i in test_ind )
